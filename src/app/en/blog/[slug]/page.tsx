/**
 * English Blog Post Detail Page
 * Shows a single English blog post
 */
...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://gamzetango.com/en/blog/${slug}`,
    ...
    alternates: {
      canonical: frontmatter.seo?.canonicalUrl || `https://gamzetango.com/en/blog/${slug}`,
      languages: {
        'tr-TR': `/blog/${slug}`,
        'en-US': `/en/blog/${slug}`,
      },
...
function generateArticleSchema(post: NonNullable<Awaited<ReturnType<typeof getPostBySlug>>>) {
  ...
  return {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://gamzetango.com/en/blog/${slug}`,
    },
...
export default async function EnglishBlogPostPage({ params }: BlogPostPageProps) {
  ...
  const relatedPosts = await getRelatedPosts(slug, 'en');
  const { previous, next } = await getAdjacentPosts(slug, 'en');
          ...
          <Container>
            <div className="grid lg:grid-cols-[1fr_280px] gap-8 lg:gap-12">
              ...
              <aside className="hidden lg:block">
                <div className="sticky top-24 space-y-8">
                  {/* Table of Contents */}
                  {post.toc.length > 0 && (
                    <TableOfContents items={post.toc} locale="en" />
                  )}
                </div>
              </aside>
            </div>
            ...
            {relatedPosts.length > 0 && (
              <section className="mt-16 pt-8 border-t border-white/10">
                <h2 className="heading-sm text-white mb-8">Related Posts</h2>
